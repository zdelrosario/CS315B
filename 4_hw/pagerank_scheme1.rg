...
do
  ...
  for link in r_links do
    ...
    -- var page_in  = r_pages_in[link.ptr_in]
    -- var page_out = r_pages_out[link.ptr_out]
    -- page_in.sum += page_out.rank / page_out.n_out
  end
end
...
task toplevel()
  ...
  var p_pages_out = image(r_pages, p_links, r_links.ptr_out)
  var p_pages_in  = image(r_pages, p_links, r_links.ptr_in)
  ...
  while not converged do
    ...
    for i = 0, config.parallelism do
      pr_reduce(p_pages_out[i],p_pages_in[i],p_links[i])
    end
    -- Compute new ranks
    __demand(__parallel)
    for i = 0, config.parallelism do
